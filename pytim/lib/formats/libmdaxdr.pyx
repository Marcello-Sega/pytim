...
cdef extern from 'include/xdrfile_trr.h':
    ...
    int read_trr(XDRFILE *xfp, int natoms, int *step, float *time, float *_lambda,
                 matrix box, rvec *x, rvec *v, rvec *f, rvec* vir, int *has_prop)
    int write_trr(XDRFILE *xfp, int natoms, int step, float time, float _lambda,
                  matrix box, rvec *x, rvec *v, rvec *f, rvec * vir)
...
TRRFrame = namedtuple('TRRFrame', 'x v f vir box step time lmbda hasx hasv hasf hasvir')
...
cdef class TRRFile(_XDRFile):
    ...
    def read(self):
        ...
        cdef np.ndarray virials = np.empty((self.n_atoms, DIMS), dtype=DTYPE)
        ...
        has_vir = bool(has_prop & HASVIR)
        return TRRFrame(xyz, velocity, forces, virials, box, step, time, lmbda,
                        has_x, has_v, has_f, has_vir)

    def write(self, xyz, velocity, forces, virials, box, int step, float time,
        ...
        virials: array_like, shape=(`n_atoms`, 3), optional
            cartesion virials. Only written if not ``None``.
        ...
        cdef np.ndarray virials_helper
        ...
        if virials is not None:
            ...
            virials_helper = np.ascontiguousarray(virials, dtype=DTYPE)
            virials_ptr = <float*>virials_helper.data
        ...
        if self.current_frame == 0:
            ...
            if virials is not None and self.n_atoms != virials.shape[0]:
                raise IOError('Previous frames virials contained {} atoms. You '
                              'are trying to write {} atoms.'.format(
                                  self.n_atoms, virials.shape[0]))
