...
for surf in [0,1] :
    # we need to clip the triangulation to the basic cell, but ax.set_xlim() does not work for 3d plots in matplotlib.
    for axis in [0,1]:
        points[surf][:,2][points[surf][:,axis]<0] = np.nan
        points[surf][:,2][points[surf][:,axis]>box[axis]] = np.nan
    # create a matplotlib Triangulation (mtmplt_tri does not accept Delaunay objects, but his owns)
    triang =  mtplt_tri.Triangulation(x=points[surf][:,0], y=points[surf][:,1], triangles=tri[surf].simplices)
    # plot the triangulation of each of the two surfaces
    ax.plot_trisurf(triang,points[surf][:,2],linewidth=0.2, antialiased=True)
