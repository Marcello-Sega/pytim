    ...
    def _define_cluster_group(self):
        ...
        if (self.cluster_cut is not None):
            ...
            if self.n_clusters is not None:
                self.cluster_group = self.universe.atoms[:0]
                self._unique_labels = list(range(min(self.n_clusters, len(unique_labels))))
                for el in self._unique_labels:
                    self.cluster_group += self.universe.atoms[
                        self.universe.atoms.clusters == el
                    ]
