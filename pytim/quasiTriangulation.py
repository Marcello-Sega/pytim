...
class QuasiTriangulation():
    ...
    def triangulation(self):
        ...
        # 4) looking for incidenty -> added point intersect touching sphere of
        #    some tetrahedron in triangulation
        sortedListOdRadii = self.initialize()
        center, face, facetmp = [np.zeros(3)] * 3
        shared, incidentFaces, incidents = [[]] * 3
        index, faceIndex, counter, cellIndex = [0] * 4
        used = False
        ...
        for pi in sortedListOdRadii:
            ...
            for face in incidentFaces:
                ...
                self.simplices = np.append(
                    self.simplices, [
                        np.array(
                            [face[0], face[1], face[2], index], dtype=np.int)
                    ],
                    axis=0)
                ...
                self.touchingCenter = np.append(
                    self.touchingCenter, [center], axis=0)
    ...
    def findNeighbors(self):
        ...
        for i in range(0, len(self.simplices)):
            ...
            self.neighbors = np.append(
                self.neighbors,
                [np.array([-2, -2, -2, -2, -2, -2, -2, -2], dtype=np.int)],
                axis=0)
    ...
    def makeListOfIncidents_old(self, index):
        ...
        dist = 0
        for i in range(0, len(self.simplices)):
            dist = math.sqrt((sum((self.points[index] - self.touchingCenter[i])
                                  **2)))
    ...
    def initialize(self):
        ...
        for i in range(0, len(self.weights)):
            ...
            self.weights[i] = self.weights[i] / 10.0
            listOfRadii.append(
                [i, self.weights[i],
    ...
    def generateFirstTetrahedron(self, CoM):
        ...
        self.points = np.append(
            self.points, [np.array(tetraPoints[0])], axis=0)
        self.points = np.append(
            self.points, [np.array(tetraPoints[1])], axis=0)
        self.points = np.append(
            self.points, [np.array(tetraPoints[2])], axis=0)
        self.points = np.append(
            self.points, [np.array(tetraPoints[3])], axis=0)
        ...
        while (isOutside):
            ...
            tetraCoM = self.points[len(self.points) - 4, :] + self.points[len(
                self.points) - 3, :] + self.points[len(
                    self.points) - 2, :] + self.points[len(self.points) - 1, :]
            tetraCoM /= 4.0
            self.touchingRadii[0] = tsphere(
                self.simplices[0][0], self.simplices[0][1],
                self.simplices[0][2], self.simplices[0][3], self.points,
                self.weights, self.touchingCenter[0])
