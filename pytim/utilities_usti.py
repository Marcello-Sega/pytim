...
#########################################################
#MODIFICATION OF ORIGINAL PYTIM UTILITIES SOURCE
#########################################################
...
def PBC(vector,box):
    if(vector[0]<-box[0]/2.0):
        vector[0]+=box[0]
    if(vector[0]>box[0]/2.0):
        vector[0]-=box[0]
    if(vector[1]<-box[1]/2.0):
        vector[1]+=box[1]
    if(vector[1]>box[1]/2.0):
        vector[1]-=box[1]
    if(vector[2]<-box[2]/2.0):
        vector[2]+=box[2]
    if(vector[2]>box[2]/2.0):
        vector[2]-=box[2]
...
def generate_periodic_border(points, box, delta,periodicity,method='3d'):
    ...
    for shift in shifts:
        if(np.sum(shift * shift)):  # avoid [0,0,0]
            #if(not periodicity[0] and shift[0]!=0):continue # skip copying in forbidden dimensions
            #if(not periodicity[1] and shift[1]!=0):continue
            #if(not periodicity[2] and shift[2]!=0):continue
...
def isTooLarge(a,b,c,d,box):
    ...
    if(d1>boxx2 or d2>boxx2 or d3>boxx2 or d4>boxx2 or d5>boxx2 or d6>boxx2 ):
        return True
    if(d1>boxy2 or d2>boxy2 or d3>boxy2 or d4>boxy2 or d5>boxy2 or d6>boxy2 ):
        return True
    if(d1>boxz2 or d2>boxz2 or d3>boxz2 or d4>boxz2 or d5>boxz2 or d6>boxz2 ):
        return True
    return False
